...
pub async fn download_latest_artifact_version(
    ...
    let ArtifactSource::Portal { slug } = artifact.source() else {
        anyhow::bail!("artifact does not have a portal source");
    };

    let latest_version = get_latest_artifact_version(http, slug)
        .await
        .context("get latest artifact version")?;

    let stored_hash = version::get_stored_version_hash(slug)
        .await
    ...
    if should_download {
        ...
        utils::download_file(
            ...
            format!(
                ...
                slug, latest_version.track, latest_version.id
            ),
            &output_path,
        )
        .await
        ...
        version::set_stored_version_hash(slug, &latest_version.version_hash)
            .await
